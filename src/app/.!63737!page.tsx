...
import { Terminal, Github, Mail, Cpu, Database, Brain, Server, Activity } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    if (terminalIndex < terminalCommands.length) {
      const timer = setTimeout(() => {
        setTerminalText(prev => prev + terminalCommands[terminalIndex] + "\n");
  ...
  const skills = [
    { name: "ML Engineering", icon: Brain, color: "text-purple-400", bg: "bg-purple-500/20" },
    { name: "AI Engineering", icon: Cpu, color: "text-cyan-400", bg: "bg-cyan-500/20" },
    { name: "Data Engineering", icon: Database, color: "text-emerald-400", bg: "bg-emerald-500/20" },
    { name: "DevOps", icon: Server, color: "text-orange-400", bg: "bg-orange-500/20" },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {skills.map((skill, index) => (
            <motion.div
              ...
              className="group"
            >
              <Card className="bg-black/50 border border-green-500/30 hover:border-green-500/60 transition-all duration-300 backdrop-blur-sm">
                <CardContent className="p-6 text-center">
                  <div className={`w-16 h-16 mx-auto mb-4 rounded-full ${skill.bg} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                    <skill.icon className={`h-8 w-8 ${skill.color}`} />
                  </div>
                  <h3 className={`text-xl font-semibold mb-2 ${skill.color}`}>{skill.name}</h3>
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <motion.div
                      initial={{ width: 0 }}
                      whileInView={{ width: "85%" }}
                      transition={{ duration: 1, delay: index * 0.2 }}
                      className={`h-2 rounded-full ${skill.bg.replace('bg-', 'bg-').replace('/20', '')}`}
                    />
                  </div>
                </CardContent>
              </Card>
